...
// সার্ভার কম্পোনেন্ট - "use client" সরানো হয়েছে
...
const PurchaseHistoryPage = ({ historyData = [], tokenData, userData }) => {
  ...
  const history = extractHistoryData(historyData);
  const user = userData || null;

  // ✅ Safe total calculation with optional chaining
  const totalSpent = history?.reduce?.((sum, item) => {
    const amount = item?.amount || 0;
    return sum + amount;
  }, 0) ?? 0;

  // ✅ Safe last purchase date
  const lastPurchaseDate = history?.length > 0 && history[0]?.timestamp 
              ...
              <table className="w-full">
                ...
                <tbody>
                  {history.map((item, index) => {
                    // ✅ Safe key generation
                    const itemId = item?._id || item?.sessionId || item?.paymentIntentId || `history-${index}`;
                    return (
                      <tr
                        key={itemId}
                        ...
                        <td className="px-4 sm:px-6 py-4 sm:py-6">
                          <div>
                            ...
                            <p className="text-xs text-gray-400 mt-1">
                              ID: {item?.sessionId?.slice(0, 8) || 'N/A'}
                            </p>
                          </div>
                        </td>
                  ...
                  >
                    ...
                    <div className="flex justify-between items-center text-sm">
                      <div>
                        ...
                        <p className="font-semibold text-[#1f2430] text-lg">
                          ${item?.amount?.toFixed(2) || '0.00'}
                        </p>
                      </div>
